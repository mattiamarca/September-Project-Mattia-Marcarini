...
if(!file.exists(gtf_file)) {
  message("⬇️ Downloading GTF from Ensembl...")
  download.file(
...
# Printing info
cat("✅ Total genes:", length(gr_all), "\n")
cat("🧬 Coding genes:", length(gr_coding), "\n")
cat("🌀 Noncoding genes:", length(gr_noncoding), "\n")

# Saving  subsets
save(gr_coding, file = "data/gr_coding.RData")
save(gr_noncoding, file = "data/gr_noncoding.RData")
cat("💾 Files saved in data/: gr_all.RData, gr_coding.RData, gr_noncoding.RData\n")
...
if(length(common_genes) == 0){
  stop("❌ No gene in common between the two datasets. Check platform/annotation.")
}
...
if(dist_centroids > 5){
  cat("⚠️ Likely relevant batch effect.\n")
} else {
  cat("✅ Minimal batch effect.\n")
}
...
# ----- 2️⃣ Load GenomicRanges -----
cat("✅ Loading GenomicRanges...\n")
load("data/gr_coding.RData")      # coding genes
load("data/gr_noncoding.RData")   # noncoding genes
cat("✅ GenomicRanges loaded successfully\n")

# ----- 3️⃣ Load dds_combat object -----
cat("✅ Loading dds_combat...\n")
load("results/dds_combat.RData")  # contains dds_combat
cat("✅ dds_combat loaded successfully\n")
...
# ----- 5️⃣ Extract top 5 significant DE genes -----
cat("✅ Cleaning gene IDs and selecting top 5...\n")
...
cat("✅ Top 5 genes selected\n")

# ----- 6️⃣ Function for violin plot -----
...
# ----- 7️⃣ Generate plots for top 5 -----
cat("✅ Generating top 5 violin plots...\n")
...
cat("✅ Top 5 violin plots saved in results/\n")
