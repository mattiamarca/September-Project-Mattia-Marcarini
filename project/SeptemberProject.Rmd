...
1. **Gene Annotation** – preparation of genomic annotations from Ensembl GTF files.  
2. **PCA** – exploratory analysis to assess the presence of batch effects.  
3. **Batch Correction & Differential Expression** – batch effect correction with *ComBat-seq* and DE analysis with DESeq2.  
4. **Heatmap & Clustering** – visualization of the most variable genes after correction.  
5. **Coding vs Noncoding Genes** – comparison and visualization of top DE genes with violin plots.   
...
cat("✅ Heatmap saved in results/heatmap_top100.pdf\n")
# Show heatmap inline
pheatmap(mat,
annotation_col = anno,
show_rownames = FALSE,
...
- Using command prompt navigate to the project folder (which should contain `SeptemberProjectScript.R`, `SeptemberProject.rmd`, `data/`, `Dockerfile`, ) with:
  cd /path/to/project
...
- Be sure that into 'data' folder are present the two datasets (GSE244485_raw_counts.tsv and GSE244486_raw_counts.tsv). If they aren't, download them   from these links ( GSE244486: https://www.ncbi.nlm.nih.gov/geo/download/?acc=GSE244486&format=file&file=GSE244486%5Fraw%5Fcounts%2Etsv%2Egz ; 
                   GSE244485: https://www.ncbi.nlm.nih.gov/geo/download/?acc=GSE244485&format=file&file=GSE244485%5Fraw%5Fcounts%2Etsv%2Egz) 
